...
//-------------------------------------------------------------------------------------------------
template <class T> inline
typename NkVector<T>::Iterator NkVector<T>::Begin()
{
	return Iterator(GetFrontDataPtr());
}

template <class T> inline
typename NkVector<T>::Iterator NkVector<T>::End()
{
	return Iterator(GetBackDataPtr() + 1);
}

template <class T> inline
typename NkVector<T>::ReverseIterator NkVector<T>::RBegin()
{
	return ReverseIterator(GetBackDataPtr());
}

template <class T> inline
typename NkVector<T>::ReverseIterator NkVector<T>::REnd()
{
	return ReverseIterator(GetFrontDataPtr() - 1);
}
...
template <class T> inline
void NkVector<T>::Remove(Iterator& kIter)
{
	T* pData = kIter.m_pData;
	unsigned int uiCount = &m_pBase[m_uiCount] - pData - 1;

	if( pData < m_pBase || uiCount < 0 )
		return;

	memmove(pData, pData + 1, sizeof(T) * uiCount);
}
