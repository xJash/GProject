...
CGash::~CGash()
{
	...
	// 잠시 기다려준다.
	//Sleep( 1000 );
	DWORD dwWaitObject = WaitForSingleObject(m_hThread, 2 * 1000);		// 최대 2초 동안 기다린다.
...
//	HANDLE hThread;
	...
	m_hThread = (HANDLE) _beginthreadex( NULL, 0, gash_worker, (LPVOID)this, 0, &uiThreadID );
//	CloseHandle( hThread );	
}
