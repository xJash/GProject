...
void NGraphics::PutSpr16EFT( int x, int y, UI08 *pSrcImage, int width, int height, int effectf, UI08 *pDestImage )
{
	...
	pEffectTable = (RGB16 *)m_pEffectFTable[ effectf ];
	...
	for( i = 0; i < height; ++i ) {
		...
		while( j < width ) {
			...
			if( *psrc == COLOR_TRANS ) {
				...
				pdest[ j ] = pEffectTable[ pColorTable[ *psrc ] ];
				++psrc;
				++j;
			}
		}
...
putpixel:
			// pdest[ ebx ] = pEffectTable[ pColorTable[ *psrc ] ]

			xor		eax, eax
			mov		ax, word ptr [esi+ecx*2]			// color table에서 rgb값을 읽어온다 
			mov		ecx, pEffectTable
			lea		ecx, [ ecx + eax * 2 ]
			mov		ax, word ptr [ecx]
...
void NGraphics::PutSpr16REFT( int x, int y, UI08 *pSrcImage, int width, int height, int effectf, UI08 *pDestImage )
{
...
void NGraphics::PutSpr16CEFT( int x, int y, UI08 *pSrcImage, int width, int height, int effectf, UI08 *pDestImage )
{
...
void NGraphics::PutSpr16CREFT( int x, int y, UI08 *pSrcImage, int width, int height, int effectf, UI08 *pDestImage )
{
			...
			for( i = 0; i < trim_y2; ++i ) {
				...
				while( j >= 0 ) {
					...
					if( *psrc == COLOR_TRANS ) {					
						...
						if( i >= trim_y1 && j >= trim_x ) {
							pdest[ j ] = pSrceffectfTable[ pColorTable[ *psrc ] ] +
										 pDesteffectfTable[ pdest[ j ] ];
						}
						++psrc; --j;
					}
				}
