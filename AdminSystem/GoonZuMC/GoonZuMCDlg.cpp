...
BOOL CGoonZuMCDlg::OnInitDialog()
{
	...
	m_dwLastDisconnectErrorTick = GetTickCount() - 40000;
	...
	m_ServerStateDlg.Create( IDD_SERVERSTATEDLG, &m_tabMain );
	...
	m_LogDlg.Create( IDD_LOGDLG , &m_tabMain );

	//m_hWndTabDialog[ 0 ] = m_ServerStateDlg.m_hWnd;
	...
	//m_hWndTabDialog[ 4 ] = m_LogDlg.m_hWnd;

	//char *str[] = { "ServerState", "Login Server Group", "Game Server Group", "ConnectedUserGraph", "Log" };
	char *str[] = { "Login Server Group", "Game Server Group", "ConnectedUserGraph" };
	...
	//TabCtrl_SetCurFocus( m_tabMain.m_hWnd, 0 );
	...
	if( nIDEvent == 1 ) {
		...
		if( m_bError ) {
			...
			if( m_errorCode & 2 ) {
				...
				sprintf( buffer, "[%02d:%02d:%02d] Check Server %s !\r\n",
						systime.wHour, systime.wMinute, systime.wSecond,
						(LPCSTR)m_strErrorServer );
		...
		if( pApp->m_pMasterProcess->GetSession()->GetState() != SESSION_STATE_ESTABLISHED ) {
			...
			sprintf( buffer, "[%02d:%02d:%02d] Disconnected Master Server!\r\n",
				systime.wHour, systime.wMinute, systime.wSecond );

			if( GetTickCount() - m_dwLastDisconnectErrorTick > 10000 ) {
				
				m_dwLastDisconnectErrorTick = GetTickCount();
...
void CGoonZuMCDlg::SendMail()
{
	...
	CString sender = "warmfriend@ndoors.net";
	CString recipient = "";
	CString subject = "";
