#include "NGuildDungeonDlg_New.h"
...
CNGuildDungeonDlg_New::CNGuildDungeonDlg_New()
{
	Init();
}
...
void CNGuildDungeonDlg_New::Init()
{
	m_siItemUnique = 0;

}

void CNGuildDungeonDlg_New::Destroy()
{

}

void CNGuildDungeonDlg_New::Create()
{
	if ( IsCreate() ) return;
	...
	file.CreatePopupDialog( this, NGUILDDUNGEON_NEW_DLG, "dialog_guilddungeon", StaticGuildDungeonDlg_NewProc );
	...
	CButton* pButton_Apply = m_InterfaceFactory.GetButton(NGUILDDUNGEON_BUTTON_APPLY)	;
	pButton_Apply->Show(false);
...
void CALLBACK CNGuildDungeonDlg_New::StaticGuildDungeonDlg_NewProc(CControllerMgr *pControllerMgr, UINT nEvent, int nControlID, CControllerObj *pControl)
{
	CNGuildDungeonDlg_New *pThis = (CNGuildDungeonDlg_New*)pControllerMgr;
	pThis->NGuildDungeonDlg_NewProc( nEvent, nControlID, pControl);
}

void CNGuildDungeonDlg_New::NGuildDungeonDlg_NewProc(UINT nEvent, int nControlID, CControllerObj* pControl)
{
...
void CNGuildDungeonDlg_New::Action()
{
}

void CNGuildDungeonDlg_New::DrawDialogLast( SI32 ScreenRX, SI32 ScreenRY )
{
	if ( m_siItemUnique > 0 )
	{
		SI32 itemType = 0;
...
void CNGuildDungeonDlg_New::ShowDungeonList()
{
	cltMsg clMsg(GAMEMSG_REQUEST_GUILDDUNGEON_STATUS, 0 );
	pclClient->SendMsgToServer((sPacketHeader*)&clMsg);	
}

void CNGuildDungeonDlg_New::SetList(cltGameMsgResponse_GuildDungeon_Status* pclinfo )
{
...
void CNGuildDungeonDlg_New::ShowItemList()
{
