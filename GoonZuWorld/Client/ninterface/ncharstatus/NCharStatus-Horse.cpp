...
BOOL CALLBACK cltNCharStatus::CharStatusHorseStaticProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
...
BOOL CALLBACK cltNCharStatus::CharStatusHorseProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	//SI32 i;
	cltClient* pclclient = (cltClient*)pclClient;
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON_STATUS_CHANGENAME:
			{
				...
				if ( pclhorse->szName != NULL && strlen( pclhorse->szName ) > 0 && strcmp( pclhorse->szName, ChangeHorseName ) == 0 )
				{
					...
					pclclient->m_pNotifyMsgDlg->SetText( pTitle, pText );
					return FALSE;
				}
				...
				if(pclChar->pclCI->clHorseInfo.siCurrentHorseRIDEHORSE == false)
				{
					...
					{	
						if(pclChar->pclCI->clHorseInfo.CanRide(0, &pclclient->clClientDate) == true)
						{
							...
							{
								cltGameMsgResponse_RideOrUnrideHorse clinfo(0);
		...
		case IDC_BUTTON_HORSE_EXPLAIN:
			{
				//pclHorseBreedExplain->CreateDlg(pclClient->GetInstance(), pclClient->GetHwnd());
		...
		case IDC_BUTTON_HORSE_ADDSTATUS:
			{
				...
				if ( pclhorse->siHorseUnique <= 0 )
				{
					...
					pclclient->m_pNotifyMsgDlg->SetText( pTitle,buffer);
					return TRUE ;
				}
...
// 말의 상태를 업데이트 한다.
void cltNCharStatus::UpdateHorseStatusInfo(HWND hDlg)
{
	HWND hwnd;
	char buffer[256];
