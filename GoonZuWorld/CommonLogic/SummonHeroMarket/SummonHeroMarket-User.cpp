...
void cltSummonHeroMarketManager::ShowMySummonInfo( HWND hDlg )
{
...
void cltSummonHeroMarketManager::ShowMyOrderInfo( HWND hDlg )
{
	if( siMyOrderSelectedIndex < 0 ||
		siMyOrderSelectedIndex >= MAX_SUMMONHEROMARKET_ORDER_PER_PERSON ) return;
...
// 수수료 정보를 보여준다.
BOOL cltSummonHeroMarketManager::ShowFee(HWND hDlg)
{
	char buffer[256];
	float feerate = (float)clClientStrInfo.clSummonHeroMarketStrInfo.clFee.GetFeeRate() / 10;
...
// 실제로 지불해야 할 금액을 보여준다. 
BOOL cltSummonHeroMarketManager::ShowRealFee(HWND hDlg, SI32 price)
{
	char buffer[256];

	sprintf(buffer, "%d", clClientStrInfo.clSummonHeroMarketStrInfo.clFee.CalcFee( price ));
...
void cltSummonHeroMarketManager::ShowMySummonList(HWND hDlg)
{
...
void cltSummonHeroMarketManager::ShowMyOrderList(HWND hDlg)
{
