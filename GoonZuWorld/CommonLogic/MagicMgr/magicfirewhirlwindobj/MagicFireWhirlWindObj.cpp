#include "MagicFireWhirlWindObj.h"
...
CMagicFireWhirlWindObj::CMagicFireWhirlWindObj( CMagicMgr *pMagicMgr ) : CMagicBaseObj( pMagicMgr )
{
...
CMagicFireWhirlWindObj::~CMagicFireWhirlWindObj()
{
	Destroy();
}

void CMagicFireWhirlWindObj::Initialize()
{
...
void CMagicFireWhirlWindObj::Destroy()
{
	if ( m_pMagicInfo )
	{
		delete m_pMagicInfo;
...
bool CMagicFireWhirlWindObj::SetMagic( stMagicInfo *pstMagicInfo, SI32 si32AddDamage, cltMapCommon *pclMap, SI32 si32SkillLevel, SI32 siOrganUnique )
{
...
// 디레이 주의 할것
bool CMagicFireWhirlWindObj::Action()
{	
...
void CMagicFireWhirlWindObj::Draw( )
{	
	...
	SetMagicSpr( GIMG_MAGIC_FIREWHIRLWIND );				// Image index
	...
	m_si32DrawYPos = screeny - GetMagicSpr()->GetYSize()*2/3 -40 ;

	GP.PutSprScreen(GetMagicSpr(), m_si32DrawXPos, m_si32DrawYPos, GetCurrentFrame() % GetMagicSpr()->GetImageNum());

	if ( GetCurrentFrame() == GetMaxFrameNum() - 1 )
	{
		SHORT siRealX, siRealY;
		SI32 siDrawX, siDrawY;
		siDrawX = screenx;
		siDrawY = screeny-70;

		pclmap->TransFromScreenToRealXY(siDrawX, siDrawY, siRealX, siRealY);
		pclClient->pclSmokeManager->SetSmoke(SMOKE_FIREWHIRLWIND_HIT,siRealX,siRealY,pclmap) ;
	}
...
// Push
bool CMagicFireWhirlWindObj::Push( BYTE *pData, SI32 si32Size )
{
...
void CMagicFireWhirlWindObj::SetDamage( SI32 si32Damage )
{
