#include "MagicDropGreatWallAsuraObj.h"
...
CMagicDropGreatWallAsuraObj::CMagicDropGreatWallAsuraObj( CMagicMgr *pMagicMgr ) : CMagicBaseObj( pMagicMgr )
{
...
CMagicDropGreatWallAsuraObj::~CMagicDropGreatWallAsuraObj()
{
	Destroy();
}

void CMagicDropGreatWallAsuraObj::Initialize()
{
...
void CMagicDropGreatWallAsuraObj::Destroy()
{
	if ( m_pMagicInfo )
	{
		delete m_pMagicInfo;
...
bool CMagicDropGreatWallAsuraObj::SetMagic( stMagicInfo *pstMagicInfo, SI32 si32AddDamage, cltMapCommon *pclMap, SI32 si32SkillLevel, SI32 siOrganUnique )
{
...
// 딜레이 주의 할것
bool CMagicDropGreatWallAsuraObj::Action()
{	
	...
	{
		if ( GetCurrentFrame() == (GetMaxFrameNum() - 7) && GetCurrentMagicDelayFrame() == 0 )		
		{
...
void CMagicDropGreatWallAsuraObj::Draw( )
{	
	...
	SetMagicSpr( GIMG_MAGIC_GREATWALLASURA );				// Image index - 하늘에서 떨어지는 석상.
	...
	m_si32DrawYPos = screeny - GetMagicSpr()->GetYSize()*6/7;


	GP.PutSpr(GetMagicSpr(), m_si32DrawXPos, m_si32DrawYPos, GetCurrentFrame() );  // PutSprScreen(투명모드)
	return;
}


// Push
bool CMagicDropGreatWallAsuraObj::Push( BYTE *pData, SI32 si32Size )
{
...
void CMagicDropGreatWallAsuraObj::SetDamage( SI32 si32Damage )
{
