...
CMagicEarthQuakeObj::CMagicEarthQuakeObj( CMagicMgr *pMagicMgr ) : CMagicBaseObj( pMagicMgr )
{
...
CMagicEarthQuakeObj::~CMagicEarthQuakeObj()
{
	Destroy();
}

void CMagicEarthQuakeObj::Initialize()
{
...
void CMagicEarthQuakeObj::Destroy()
{
	if ( m_pMagicInfo )
	{
		delete m_pMagicInfo;
...
bool CMagicEarthQuakeObj::SetMagic( stMagicInfo *pstMagicInfo, SI32 si32AddDamage, cltMapCommon *pclMap,SI32 si32SkillLevel, SI32 siOrganUnique )
{
	/*
	//---------------------------------
	// 맞는 사람의 아이디를 구한다. 
	//---------------------------------
	SI32 toid	= pclClient->pclCM->GetIDFromCharUnique(pstEarthQuakeInfo->sHeader.si32ToID);
...
// 디레이 주의 할것
bool CMagicEarthQuakeObj::Action()
{	
...
//			pclclient->pclMusic->PushEffect( EFFECT_MAGIC_EARTHQUAKE_FIRE, pclClient->pclCM->CR[id]->GetX(), pclClient->pclCM->CR[id]->GetY() );
		}		
	}
...
{	
	...
	SetMagicSpr( GIMG_MAGIC_EARTHQUAKE );				// Image index
	...
	m_si32DrawYPos = screeny - GetMagicSpr()->GetYSize()/2 - 20	;
...
// Push
bool CMagicEarthQuakeObj::Push( BYTE *pData, SI32 si32Size )
{
...
void CMagicEarthQuakeObj::SetDamage( SI32 si32Damage, SI32 si32CurrentRadius )
{
