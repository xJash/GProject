...
void NGraphics::PutMpr16AT( int x, int y, UI08 *pSrcImage, int width, int height, int alpha, UI08 *pDestImage )
{
	...
	for( i = 0; i < height; ++i ) {
		...
		while( j < width ) {
			...
			if( *psrc == COLOR_TRANS ) {
				...
				for( ; k < j; ++k ) {
					pdest[ k ] = pSrcAlphaTable[ pColorTable[ *psrc ] ] +
...
void NGraphics::PutMpr16CAT( int x, int y, UI08 *pSrcImage, int width, int height, int alpha, UI08 *pDestImage )
{
