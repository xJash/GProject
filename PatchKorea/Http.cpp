...
bool CHttp::Connect()
{
	...
	char * pText = TEXT("Error5, Inspect Your Internet Connection, Please.\r");
...
bool CHttp::HttpFileConnect( char* strURL, char* strLocalFileName )
{
	...
	char *pText = TEXT("Error6, Inspect Your Internet Connection, Please.\r");
	if ( m_hURL == NULL )
	{
		AfxMessageBox(pText);
		return false;
	...
	{
		pText = TEXT("1 After a while, Try to Reconnect, Please.\r");
		AfxMessageBox(pText);
		return false;
	}
...
void CHttp::FileRecv()
{
	...
	while( m_bThreadRun )						// m_bThreadRun 은 FileRecv 를 호출하는 쓰레드 공통으로 쓰이는 변수 이므로, 이 변수 값이 도중에 false로 변하면 모든 쓰레드가 중지됨.
	{
		...
		if ( dwSize > 1000000 )
		{
			char *pText = TEXT("Please,Download the Total installation file at the web and Try to install \r Error Internet speed access.");
			AfxMessageBox(pText);
